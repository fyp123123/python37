...
#     '''
#     def __int__(self,filname):
#         super(Handle_Conf,self).__init__()#继承父类
#         self.filname = filname #把传入进来的形式参数赋值给实列变量 self，filname
...
#         return  value
#
# path = os.path.join(conf_path,'conf.ini')#拼接conf.ini文化路径
# conf = Handle_Conf(path) #创建对应吧conf.ini文件作为入参
# # print(conf.get_value('env','url'))



from configparser import  ConfigParser  #用来读取ini文件
from utlis.handle_path import *
import os
...
# print(conf.get_value('env','headers'))  #通过conf对象调用get_value传入section核option拿到value值
